    ...
    <AdminLayout>
      <div class="container mx-auto py-8">
        <div class="bg-white dark:bg-gray-800 shadow-lg rounded-lg p-8">
          <h1 class="text-3xl font-bold mb-6 text-gray-800 dark:text-gray-300">Edit Expense Category</h1>
...
onMounted(() => {
  form.name = props.expenseCategory.name; // Initialize form with existing expense category data
});

function updateExpenseCategory() {
  form.put(route('admin.expense-categories.update', props.expenseCategory.id), {
    onSuccess: () => {
      successMessage.value = 'Expense Category updated successfully!';
      errorMessage.value = ''; // Clear any previous error messages
    },
    onError: () => {
      errorMessage.value = 'Error updating category. Please try again.';
