<template>
    ...
    <AdminLayout>
      <div class="container mx-auto py-8">
        <div class="bg-white dark:bg-gray-800 shadow-lg rounded-lg p-8">
          <h1 class="text-3xl font-bold mb-6 text-gray-800 dark:text-gray-300">Edit Expense</h1>
  ...
  const props = defineProps({
    ...
    expense: Object, // Existing expense data passed from the server
  });
  ...
  function updateExpense() {
    form.put(route('admin.expenses.update', props.expense.id), {
      onSuccess: () => {
        successMessage.value = 'Expense updated successfully!';
        errorMessage.value = ''; // Clear any previous error messages
      },
      onError: () => {
        errorMessage.value = 'Error updating expense. Please try again.';
